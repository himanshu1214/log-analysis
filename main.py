...
#Popular Articles:
    def get_popular_articles(self):
        ...
        '''
        self.conn.execute("SELECT count(*) as num, A.slug FROM articles AS A INNER JOIN log AS B ON B.path \
        like '%'|| A.slug || '%' group by A.slug;")
...
# Popular Articles
    def get_popular_author_name(self):
        ...
        '''
        self.conn.execute("CREATE OR REPLACE VIEW aut_art AS SELECT A.slug, A.author, B.name \
        FROM articles AS A, authors AS B WHERE A.author=B.id;")
        self.conn.execute("SELECT A.name, count(*) as num FROM aut_art A INNER JOIN log AS B ON B.path \
        like '%'|| A.slug || '%' group by A.name;")
        res=self.conn.fetchall()
...
#Top request error
    def get_request_errors(self):
        '''
        This function returns the date where error more than 1% percent occured out of total requrest GET request sent
        :param: takes no parameter
        :return: df: Dataframe of %errors and Date
        '''
        view_date_tab="CREATE OR REPLACE VIEW date_tab AS SELECT date_part('day', time), status FROM log;"
        view_group_query='CREATE OR REPLACE VIEW group_tab AS SELECT count(*) as num, date_part, status \
        FROM date_tab group by date_part, status;'
        view_percent_error_tab=("CREATE OR REPLACE VIEW error_tab AS SELECT A.num/B.sum*100 error, A.date_part \
        FROM (SELECT num, date_part FROM group_tab WHERE status='404 NOT FOUND' order by date_part ASC) A, \
        (SELECT sum(num), date_part FROM group_tab group by date_part order by date_part) B \
        WHERE A.date_part=B.date_part order by error DESC;")
