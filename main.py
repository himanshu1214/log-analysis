...
#   Popular Articles:
    ...
    def get_popular_articles(self):
        ...
        '''
        self.conn.execute('''SELECT count(*) as views, title FROM log JOIN
        articles ON log.path = concat('/article/', articles.slug) group by
...
# Popular Articles
    ...
    def get_popular_author_name(self):
        ...
        '''
        Function returns the popular author names
        :param: takes no parameter
        :return: df: Dataframe of Author and Views
        '''
        self.conn.execute('''SELECT A.name, count(*) as views FROM aut_art A
        INNER JOIN log AS B ON B.path = concat('/article/', A.slug) group by
        A.name order by views DESC LIMIT 3;''')
        res = self.conn.fetchall()
...
# Top request error
    ...
    def __str__(self):
        ...
        Date with more than onePercenterror: \n {}'''.format(
            self.get_popular_articles(), self.get_popular_author_name(),
